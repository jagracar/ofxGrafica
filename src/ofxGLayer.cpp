...
ofxGLayer::ofxGLayer(const string &_id, const array<float, 2> &_dim,
	...
	if (xLog && (xLim[0] <= 0 || xLim[1] <= 0)) {
		ofLogWarning(
				"One of the limits is negative. This is not allowed in logarithmic scale. Will set horizontal limits to (0.1, 10)");
		xLim = {0.1f, 10};
	}

	if (yLog && (yLim[0] <= 0 || yLim[1] <= 0)) {
		ofLogWarning(
				"One of the limits is negative. This is not allowed in logarithmic scale. Will set vertical limits to (0.1, 10)");
		yLim = {0.1f, 10};
	}
...
ofxGPoint ofxGLayer::valueToPlot(ofxGPoint point) const {
...
bool ofxGLayer::isInside(ofxGPoint plotPoint) const {
